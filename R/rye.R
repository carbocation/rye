...
rye.loadDevelopmentNative = function(repository = getwd()) {
  libraryName = switch(
    .Platform$OS.type,
    windows = 'rye.dll',
    paste0('librye', if (Sys.info()[['sysname']] == 'Darwin') '.dylib' else '.so')
  )
  libraryPath = file.path(repository, 'target', 'release', libraryName)
  if (file.exists(libraryPath)) {
    loaded = !inherits(try(dyn.load(libraryPath), silent = TRUE), 'try-error')
...
printError = function(msg){cat(crayon::red(paste(msg, collapse = " "), "\n"))}
printWarn = function(msg){cat(crayon::yellow(paste(msg, collapse = " "), "\n"))}
logmsg = function(msg){cat(crayon::green(paste(format(Sys.time(), "[ %b %d %Y - %X ]"), msg, collapse = " "), "\n"))}
progressmsg = function(msg){cat(crayon::magenta(paste(msg, collapse = " "), "\n"))}
  ...
  if (rye.nativeSolver) {
    ...
    estimates = t(vapply(seq_len(nrow(weightedX)), function(row) {
      if (!requireNamespace('nnls', quietly = TRUE)) {
        stop("Install the nnls package or reinstall Rye with its Rust accelerator")
      }
      coefficients = coef(nnls::nnls(A = design, b = weightedX[row, ]))
...
rye.main = function(args = commandArgs(trailingOnly = TRUE)) {
  oldOptions = options(width = 220, scipen = 999, digits = 4)
  on.exit(options(oldOptions), add = TRUE)
  optParser = optparse::OptionParser(option_list = optionList)
  opt = optparse::parse_args(optParser, args = args)
  start_time <- Sys.time()
